# Polymer Moments Artifact — Normative Format Spec (`polymer_moments_format/2.3`)

**Artifact:** `polymer_pools.json`, emitted next to `chem.yaml` at the end of an
RMG run (`save_everything`). **Schema versions:** `2.0`–`2.3`, stamped per
...
implements (currently `2.0`–`2.3`) and hard-rejects anything newer — these
...
implements `2.0`–`2.3` and rejects `2.4`+.
