# Polymer Moments Artifact — Normative Format Spec (`polymer_moments_format/2.4`)

**Artifact:** `polymer_pools.json`, emitted next to `chem.yaml` at the end of an
RMG run (`save_everything`). **Schema versions:** `2.0`–`2.4`, stamped per
...
implements (currently `2.0`–`2.4`) and hard-rejects anything newer — these
...
implements `2.0`–`2.4` and rejects `2.5`+.
...
`2.0`–`2.4` and rejects `2.5`+, restoring the marker onto the reconstructed
