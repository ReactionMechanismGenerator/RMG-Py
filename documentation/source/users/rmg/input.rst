...
generated by RMG from previous run (must be RMG version >= 3.0)), speciesMap (a YAML file specifying what species are
at each index in the filters), and familyMap (a YAML file specifying what families are at each index in the filters). ::
